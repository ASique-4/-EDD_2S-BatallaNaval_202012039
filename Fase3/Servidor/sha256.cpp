...
/// process 64 bytes
void SHA2562::processBlock(const void* data)
{
  // get last hash
  uint32_t a = m_hash[0];
  uint32_t b = m_hash[1];
...
void SHA2562::add(const void* data, size_t numBytes)
{
...
void SHA2562::getHash(unsigned char buffer[SHA2562::HashBytes])
{
...
std::string SHA2562::operator()(const void* data, size_t numBytes)
{
  reset();
  add(data, numBytes);
  return getHash();
...
std::string SHA2562::operator()(const std::string& text)
{
  reset();
